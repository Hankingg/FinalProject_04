 ...
 * 
 */
 let remoteStreamElement = document.querySelector('#remoteStreamDiv');
    ...
    const createPeerConnection = (otherKey) =>{
    	...
    	console.log("createpeercon오냐")
        const pc = new RTCPeerConnection();
        try {
            if(localStream !== undefined){
            	console.log("여긴 돌아야할텐데")
                localStream.getTracks().forEach(track => {
                    pc.addTrack(track, localStream);
                });
            }
            pc.addEventListener('icecandidate', (event) =>{
            	console.log("icecandidate는 도냐")
            ...
            pc.addEventListener('track', (event) =>{
            	...
            	if (remoteTracks.length === 0) {
        	        console.log("Remote stream is empty for key: " + otherKey);
        	    } else {
        	        console.log("Remote stream is not empty for key: " + otherKey);
        	    }
            	console.log("onTrack이벤트리스너오냐")
